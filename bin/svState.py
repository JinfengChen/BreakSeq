...
os.system("%s/CheckFasta.py --input %s" % (sys.path[0], outpre))

for g in server.keys():
	print "> Aligning to ancestral genome %s and performing state analysis"%g
	g_outpre="%s.%s" % (outpre, g)
	for t in ("A", "B", "C"):
		input="%s.%s.clean.fa" % (outpre,t)
		output="%s.%s" % (g_outpre, t)
		os.system("%s -minScore=0 -minIdentity=90 %s %s %s.psl" % (client, twobit[g], input, output))
